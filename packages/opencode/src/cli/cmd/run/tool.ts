...
import type { PlanExitTool, PlanCreateTool } from "@/tool/plan"
...
type ToolDefs = {
  ...
  plan_create: typeof PlanCreateTool
}
...
function runPlanCreate(p: ToolProps<typeof PlanCreateTool>): ToolInline {
  return {
    icon: "+",
    title: text(p.frame.state.title) || "Created plan",
    mode: "block",
    body: p.frame.status === "completed" ? text(p.frame.state.output) : undefined,
  }
}
